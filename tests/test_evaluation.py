...
def test_calculate_imputation_metrics(sample_preprocessed_data):
    ...
    mask = sample_preprocessed_data['test_indicating_mask']
      ...
      # --- Robust NaN-free initialization for BOTH arrays ---
    y_true_no_nan = np.nan_to_num(y_true.copy(), nan=0.0) # Ensure NaN-free ground truth
    y_pred = np.nan_to_num(sample_preprocessed_data['test_X_ori'].copy(), nan=0.0) # Start NaN-free

    # --- End Robust NaN Handling ---

    # Simulate perfect imputation for testing metrics calculation
    y_pred[mask == 1] = y_true_no_nan[mask == 1] # Use NaN-free ground truth for copy
    
    # Pass NaN-free ground truth to metrics
    ...
    # With perfect imputation where mask is 1, errors should be zero (or very close)
    if mask.sum() > 0: # Only assert if there were NaNs to impute
        ...
        # MRE might be NaN or inf if true value is 0, handle carefully
        # assert metrics['mre'] == pytest.approx(0.0, abs=1e-6)
    else:
        assert np.isnan(metrics['mae']) # No values imputed
...
def test_calculate_imputation_metrics_imperfect(sample_preprocessed_data):
    ...
    # Simulate imperfect imputation (e.g., add noise)
    noise = np.random.randn(*y_pred.shape) * 0.1
    y_pred[mask == 1] = y_true[mask == 1] + noise[mask == 1] # Add noise only where imputed

     # Need to handle potential NaNs in y_true *before* adding noise if they exist
    ...
    # Fill remaining NaNs in prediction if any (e.g. if original data had NaNs not in mask)
...
def test_evaluate_all_methods(sample_preprocessed_data):
    ...
    # Method 2: Mean imputation (simplified for test)
